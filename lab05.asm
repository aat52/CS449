...
_sum:
	...
	slti $s0, $a0, 1	# set to 1 if less than
	beq $s0, $zero, _sumRecursion	
	...
	_sumRecursion:
		...
		add $v0, $a0, $v0	# return answer in v0
		jr $ra
...
#   $v0 = F(n)
_fibonacci:
	addiu $sp, $sp, -8	# allocate activation frame
    	sw $a0, 0($sp)
   	sw $ra, 4($sp)
	...
	one:
    		...
    		addiu $sp, $sp, -4	# allocate activation frame
    		sw $v0, 0($sp)
...
#   - $v0 = x^y
_pow:
	addi $sp, $sp, -4	# allocate activation frame
	sw $ra, 0($sp)
	
	bne $a1, $zero, continue	# check if greater than 0
	addi $v0, $zero, 1
